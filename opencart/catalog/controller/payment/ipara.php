...
class ControllerPaymentIpara extends Controller
{
	...
	public function index()
	{
		...
		$data['bank'] = nl2br($this->config->get('ipara_bank' . $this->config->get('config_language_id')));

		$data['continue'] = $this->url->link('checkout/success');

		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/ipara.tpl')) {
			return $this->load->view($this->config->get('config_template') . '/template/payment/ipara.tpl', $data);
		} else {
			return $this->load->view('default/template/payment/ipara.tpl', $data);
		}
	}
	...
	public function paymentform()
	{
		...
		if (isset($record['result']) AND $record['result']) {
			...
			$this->model_checkout_order->addOrderHistory($this->session->data['order_id'], $this->config->get('ipara_order_status_id'), $comment);
		...
		$data['rates'] = $ipara_rates;
		$data['error_message'] = $error_message;
		$data['cart_id'] = $this->session->data['order_id'];
		$data['form_link'] = $this->url->link('payment/ipara/paymentform', '', 'SSL');


		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/ipara_ccform.tpl')) {
			$this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/payment/ipara_ccform.tpl', $data));
		} else {
			$this->response->setOutput($this->load->view('default/template/payment/ipara_ccform.tpl', $data));
		}
	}
	...
	function post2Ipara()
	{
		...
		$obj->success_url = $this->url->link('payment/ipara/paymentform', 'tdvalidate=1', 'SSL');
		$obj->failure_url = $this->url->link('payment/ipara/paymentform', 'tdvalidate=1', 'SSL');
